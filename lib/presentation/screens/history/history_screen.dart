...
class _HistoryScreenState extends State<HistoryScreen> {
        ...
        actions: [
          IconButton(
            onPressed: () {
              ...
              if (!_isSearching) { _searchCtrl.clear(); provider.setSearchQuery(''); }
            },
  ...
  Widget _buildBody(BuildContext ctx, AppProvider p, List<OcrRecord> records,
      AppLocalizations l) {

    // 模型下载中
    if (p.status == AppStatus.preparingModel) {
      return _buildModelLoadingState();
    }

    // 识别中
    if (p.status == AppStatus.processing) {
      return Center(child: Column(
        ...
        children: [
          ...
          Text('PP-OCRv5 · ${l.scanLocalProcess}',
      ...
      itemBuilder: (c, i) {
        ...
        return _RecordCard(
          ...
          onTap: () {
            ...
            Navigator.push(ctx, MaterialPageRoute(builder: (_) => const ResultScreen()));
          },
          onDelete: () => _showDeleteConfirm(ctx, r, p, l),
        ).animate(delay: Duration(milliseconds: i * 40)).fadeIn().slideY(begin: 0.07);
      },
    );
  }

  Widget _buildModelLoadingState() {
    return Center(child: Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        const SizedBox(width: 48, height: 48,
            child: CircularProgressIndicator(
                color: AppTheme.accentColor, strokeWidth: 3)),
        const SizedBox(height: 20),
        const Text('Downloading PP-OCRv5 model…',
        ...
        Container(
          ...
          child: const Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            SizedBox(width: 6),
            Text('Model runs 100% locally after download',
    ...
    return Center(
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Container(
          ...
          child: const Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.auto_awesome, color: AppTheme.primaryColor, size: 14),
            SizedBox(width: 6),
            Text('PP-OCRv5 · Local · Private',
  ...
  void _showDeleteConfirm(BuildContext ctx, OcrRecord r,
      AppProvider p, AppLocalizations l) {
    showDialog(
      context: ctx,
      builder: (d) => AlertDialog(
        backgroundColor: AppTheme.darkCard,
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(d), child: Text(l.cancel)),
          TextButton(
            onPressed: () { Navigator.pop(d); p.deleteRecord(r.id); },
            child: Text(l.delete, style: const TextStyle(color: Colors.red)),
          ),
        ],
      ),
